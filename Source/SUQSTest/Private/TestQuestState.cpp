...
bool FTestQuestAcceptSimple::RunTest(const FString& Parameters) {
  ...
  TestEqual("Main quest should be unavailable", Progression->GetQuestStatus("Q_Main1"),
            ESuqsQuestStatus::Unavailable);
  ...
  TestTrue("Main quest task should be incomplete",
           Progression->IsTaskIncomplete("Q_Main1", "T_ReachThePlace"));
  TestFalse("Main quest task should not be complete",
            Progression->IsTaskCompleted("Q_Main1", "T_ReachThePlace"));

  TestEqual("Side quest should be unavailable", Progression->GetQuestStatus("Q_Side1"),
            ESuqsQuestStatus::Unavailable);
  TestTrue("Should be able to accept main quest", Progression->AcceptQuest("Q_Main1"));
  TestEqual("Main quest should be incomplete", Progression->GetQuestStatus("Q_Main1"),
            ESuqsQuestStatus::Incomplete);
  TestTrue("Should be able to accept side quest", Progression->AcceptQuest("Q_Side1"));
  TestEqual("Side quest should be incomplete", Progression->GetQuestStatus("Q_Side1"),
            ESuqsQuestStatus::Incomplete);
...
bool FTestQuestAcceptFailedComplete::RunTest(const FString& Parameters) {
  ...
  Progression->InitWithQuestDataTables(
      TArray<UDataTable*>{USuqsProgression::MakeQuestDataTableFromJSON(SmallestPossibleQuestJson)});

  // Test accepting failed
  TestTrue("Accept smallest quest", Progression->AcceptQuest("Q_Smol"));
  TestEqual("Smol quest should be incomplete", Progression->GetQuestStatus("Q_Smol"),
            ESuqsQuestStatus::Incomplete);
  Progression->FailQuest("Q_Smol");
  TestEqual("Smol quest should now be failed", Progression->GetQuestStatus("Q_Smol"),
            ESuqsQuestStatus::Failed);
  TestTrue("Accepting failed quest should succeeed (and reset)",
           Progression->AcceptQuest("Q_Smol"));
  TestEqual("Smol quest should be incomplete again", Progression->GetQuestStatus("Q_Smol"),
            ESuqsQuestStatus::Incomplete);
  ...
  TestEqual("Smol quest should now be completed", Progression->GetQuestStatus("Q_Smol"),
            ESuqsQuestStatus::Completed);
  // Suppress warning logs
  LogSUQS.SetVerbosity(ELogVerbosity::NoLogging);
  TestFalse("Accepting completed quest should do nothing by default",
            Progression->AcceptQuest("Q_Smol"));
  LogSUQS.SetVerbosity(ELogVerbosity::Verbose);

  TestEqual("Smol quest should still be completed", Progression->GetQuestStatus("Q_Smol"),
            ESuqsQuestStatus::Completed);
  TestTrue("Accepting completed quest with reset option should reset",
           Progression->AcceptQuest("Q_Smol", false, true));
  TestEqual("Smol quest should be incomplete again", Progression->GetQuestStatus("Q_Smol"),
            ESuqsQuestStatus::Incomplete);
...
bool FTestQuestReset::RunTest(const FString& Parameters) {
  ...
  TestEqual("Main quest should still be incomplete", Progression->GetQuestStatus("Q_Main1"),
            ESuqsQuestStatus::Incomplete);
...
bool FTestQuestOrderedTasks::RunTest(const FString& Parameters) {
  USuqsProgression* Progression = NewObject<USuqsProgression>();
  Progression->InitWithQuestDataTables(
      TArray<UDataTable*>{USuqsProgression::MakeQuestDataTableFromJSON(OrderedTasksQuestJson)});

  TestTrue("Accept quest should work", Progression->AcceptQuest("Q_Ordered"));
  // Suppress the warnings this will raise
  LogSUQS.SetVerbosity(ELogVerbosity::NoLogging);
  TestFalse("Should not be allowed to complete task out of order",
            Progression->CompleteTask("Q_Ordered", "T_2"));
  TestFalse("Should not be allowed to complete task out of order",
            Progression->CompleteTask("Q_Ordered", "T_3"));
  TestFalse("Should not be allowed to complete a task on a later objective",
            Progression->CompleteTask("Q_Ordered", "T_11"));
  LogSUQS.SetVerbosity(ELogVerbosity::Verbose);
  ...
  TestTrue("Completing an already completed previous task should pass but do nothing",
           Progression->CompleteTask("Q_Ordered", "T_1"));

  TestTrue("Task 3 should complete OK", Progression->CompleteTask("Q_Ordered", "T_3"));

  LogSUQS.SetVerbosity(ELogVerbosity::NoLogging);
  TestFalse("Objective 2 Task 2 is out of order still",
            Progression->CompleteTask("Q_Ordered", "T_12"));
  LogSUQS.SetVerbosity(ELogVerbosity::Verbose);

  TestTrue("Objective 2 Task 1 should complete OK now",
           Progression->CompleteTask("Q_Ordered", "T_11"));
  TestTrue("Objective 2 Task 2 should complete OK now",
           Progression->CompleteTask("Q_Ordered", "T_12"));
...
bool FTestQuestUnorderedTasks::RunTest(const FString& Parameters) {
  ...
  USuqsProgression* Progression = NewObject<USuqsProgression>();
  Progression->InitWithQuestDataTables(
      TArray<UDataTable*>{USuqsProgression::MakeQuestDataTableFromJSON(UnorderedTasksQuestJson)});

  TestTrue("Accept quest should work", Progression->AcceptQuest("Q_Unordered"));
  TestTrue("Task 3 should complete out of order OK",
           Progression->CompleteTask("Q_Unordered", "T_3"));
  TestTrue("Task 1 should complete out of order OK",
           Progression->CompleteTask("Q_Unordered", "T_1"));
  TestTrue("Task 2 should complete out of order OK",
           Progression->CompleteTask("Q_Unordered", "T_2"));
...
bool FTestQuestAny2OfTasks::RunTest(const FString& Parameters) {
  USuqsProgression* Progression = NewObject<USuqsProgression>();
  Progression->InitWithQuestDataTables(
      TArray<UDataTable*>{USuqsProgression::MakeQuestDataTableFromJSON(AnyOfTasksQuestJson)});
  ...
  TestFalse("Quest should be incomplete after 1 task complete",
            Progression->IsQuestCompleted("Q_Any2Of"));
  TestTrue("Task 2 should complete OK", Progression->CompleteTask("Q_Any2Of", "T_2"));
  TestTrue("Quest should be complete after 2 tasks complete",
           Progression->IsQuestCompleted("Q_Any2Of"));
  ...
  // Failing one of the tasks should NOT fail everything, because it's still possible to complete it
  // (2 others)
  ...
  TestTrue("Task 1 should complete OK", Progression->CompleteTask("Q_Any2Of", "T_1"));
  TestTrue("Quest should be complete after any 2 tasks complete",
           Progression->IsQuestCompleted("Q_Any2Of"));
  ...
  TestFalse("Quest should remain incomplete after 2 tasks if one is optional",
            Progression->IsQuestCompleted("Q_Any2Of"));
...
bool FTestTargetNumber::RunTest(const FString& Parameters) {
  ...
  USuqsProgression* Progression = NewObject<USuqsProgression>();
  Progression->InitWithQuestDataTables(
      TArray<UDataTable*>{USuqsProgression::MakeQuestDataTableFromJSON(TargetNumberQuestJson)});

  TestTrue("Accept quest should work", Progression->AcceptQuest("Q_TargetNumbers"));

  TestEqual("First task only has a target number of 1",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf1", 1), 0);
  TestTrue("First task should now be complete",
           Progression->IsTaskCompleted("Q_TargetNumbers", "T_TargetOf1"));

  TestEqual("Second task should be not started",
            Progression->GetTaskState("Q_TargetNumbers", "T_TargetOf3")->GetStatus(),
            ESuqsTaskStatus::NotStarted);
  TestEqual("Second task should have 2 left",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf3", 1), 2);

  TestEqual("Second task should be in progress",
            Progression->GetTaskState("Q_TargetNumbers", "T_TargetOf3")->GetStatus(),
            ESuqsTaskStatus::InProgress);
  TestEqual("Second task should have 0 left",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf3", 2), 0);
  TestTrue("Second task should be completed",
           Progression->IsTaskCompleted("Q_TargetNumbers", "T_TargetOf3"));

  TestEqual("Third task should have 4 left",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf6", 2), 4);
  TestEqual("Third task should have 3 left",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf6", 1), 3);
  // go backwards
  TestEqual("Third task should have 4 left after -1",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf6", -1), 4);
  // now go forward again, test clamping at max
  auto T3 = Progression->GetTaskState("Q_TargetNumbers", "T_TargetOf6");
  TestEqual("Third task should have 0 left",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf6", 12), 0);
  TestEqual("Third task should have clamped progress at max", T3->GetNumber(), 6);
  TestTrue("Third task should be completed",
           Progression->IsTaskCompleted("Q_TargetNumbers", "T_TargetOf6"));
  TestTrue("Quest should be complete now", Progression->IsQuestCompleted("Q_TargetNumbers"));

  // test going backwards from complete
  TestEqual("Third task should have 2 left after -2",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf6", -2), 2);
  TestEqual("Third task should be back in progress after negative",
            Progression->GetTaskState("Q_TargetNumbers", "T_TargetOf6")->GetStatus(),
            ESuqsTaskStatus::InProgress);
  ...
  TestEqual("Third task should have 6 left after -12",
            Progression->ProgressTask("Q_TargetNumbers", "T_TargetOf6", -12), 6);
...
bool FTestMultiObjective::RunTest(const FString& Parameters) {
  ...
  // I could access the task objects using the API but I want to test that the top-level interface
  // is working, which uses the nested objects anyway.
  FName TaskIDs[] = {
      "T_ReachThePlace", // O1
      "T_DoTheThing", // O1
...
bool FTestDescriptions::RunTest(const FString& Parameters) {
  ...
  TestEqual("Quest description should be incomplete version", Q->GetDescription().ToString(),
            "This is the main quest");
  TestEqual("Objective description should be incomplete version", O1->GetDescription().ToString(),
            "This is the thing you do first");
  ...
  TestEqual("Objective description should have changed to complete version",
            O1->GetDescription().ToString(), "You did the thing you do first!");
  TestEqual("Quest description should still be incomplete version", Q->GetDescription().ToString(),
            "This is the main quest");

  // Complete everything en masse
  Q->Complete();
  TestEqual("Quest description should have changed to the complete version",
            Q->GetDescription().ToString(), "You did the main quest!");
...
bool FTestContinueOnFailObjective::RunTest(const FString& Parameters) {
  USuqsProgression* Progression = NewObject<USuqsProgression>();
  Progression->InitWithQuestDataTables(
      TArray<UDataTable*>{USuqsProgression::MakeQuestDataTableFromJSON(ContinueOnFailJson)});
  ...
  TestNotNull("Next Objective should be active",
              Progression->GetCurrentObjective("Q_QuestContinue"));
