...
bool FTestQuestTimeLimitSimple::RunTest(const FString& Parameters) {
  ...
  Progression->InitWithQuestDataTables(
      TArray<UDataTable*>{USuqsProgression::MakeQuestDataTableFromJSON(TimeLimitQuestJson)});
  ...
  // Ticking past where later objectives would fail, but should not fail because only first
  // objective should tick and that task has a longer timeout
  Progression->Tick(60); // total 70
  TestFalse("Quest should not be failed", Progression->IsQuestFailed("Q_TimeLimits"));
  TestFalse("Inactive tasks should not be failed",
            Progression->IsTaskFailed("Q_TimeLimits", "T_SecondTimeLimited"));
  ...
  TestTrue("Objective should have failed due to timeout",
           Progression->IsObjectiveFailed("Q_TimeLimits", "O_Single"));
  TestTrue("Task should have failed due to timeout",
           Progression->IsTaskFailed("Q_TimeLimits", "T_Single"));
...
bool FTestQuestTimeLimitNotFirstTask::RunTest(const FString& Parameters) {
  ...
  Progression->Tick(10);
  TestEqual("Ticking while objective inactive should not have changed second objective",
            T->GetTimeRemaining(), OrigTimeLeft);
  // Complete first objective so we can start on the next
  TestTrue("Completing task should have worked",
           Progression->CompleteTask("Q_TimeLimits", "T_Single"));
  TestEqual("First task of second objective should be current",
            Progression->GetNextMandatoryTask("Q_TimeLimits")->GetIdentifier(),
            FName("T_NonTimeLimited"));
  // this task isn't time limited, and objective is sequential, so again ticking should not affect
  // the time limited task
  Progression->Tick(10);
  TestEqual("Ticking while time limited task isn't next shouldn't reduce time left",
            T->GetTimeRemaining(), OrigTimeLeft);
  // Complete the non-timelimited task
  TestTrue("Completing task should have worked",
           Progression->CompleteTask("Q_TimeLimits", "T_NonTimeLimited"));
  // OK time limited task should be active now, and should reduce
  Progression->Tick(10);
  TestTrue("Second task should be reducing time limit now",
           T->GetTimeRemaining() < (OrigTimeLeft - 9.9));
  ...
  Progression->Tick(50);
  TestEqual("Time remaining should not have changed", T->GetTimeRemaining(),
            TimeRemainingAtCompletion);
