...
void USuqsWaypointComponent::BeginPlay() {
  ...
  if (!QuestID.IsNone() && !TaskID.IsNone() && IsValid(GetWorld())) {
    ...
    UE_LOG(LogSUQS, Warning, TEXT("Waypoint component %s ignored because it is not initialised"),
           *GetReadableName());
  }
}

void USuqsWaypointComponent::OnUpdateTransform(EUpdateTransformFlags UpdateTransformFlags,
                                               ETeleportType Teleport) {
  Super::OnUpdateTransform(UpdateTransformFlags, Teleport);

  // Notify
  if (bIsCurrent && bRaiseMoveEvents) { OnWaypointMoved.Broadcast(this); }
}
