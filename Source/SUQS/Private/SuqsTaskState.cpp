...
void USuqsTaskState::Initialise(const FSuqsTask* TaskDef, USuqsObjectiveState* ObjState,
...
void USuqsTaskState::Tick(float DeltaTime) {
  ...
  if (!bHidden && IsIncomplete() && IsTimeLimited() && TimeRemaining > 0) {
  ...
  if (Status != ESuqsTaskStatus::Completed) {
    ...
    if (!ParentObjective->GetParentQuest()->IsLoading()) {
      ...
      if (ParentObjective->GetParentQuest()->GetCurrentObjective() != ParentObjective) {
        UE_LOG(LogSUQS, Verbose,
               TEXT("Tried to complete task %s but parent objective %s is not current, ignoring"),
      ...
      if (ParentObjective->AreTasksSequential()) {
        ...
        if (IsMandatory() && ParentObjective->GetNextMandatoryTask() != this) {
          UE_LOG(LogSUQS, Warning,
                 TEXT("Tried to complete mandatory task %s out of order, ignoring"),
...
bool USuqsTaskState::IsResolveBlocked() const {
  return !IsIncomplete() && ResolveBarrier.Conditions > 0 && ResolveBarrier.bPending;
}
...
USuqsWaypointComponent* USuqsTaskState::GetWaypoint(bool bOnlyEnabled) {
  if (IsValid(GetWorld())) {
    ...
    if (IsValid(GI)) {
      ...
      return Suqs->GetWaypoint(GetParentObjective()->GetParentQuest()->GetIdentifier(),
                               GetIdentifier(), bOnlyEnabled);
    }
  }
...
TArray<USuqsWaypointComponent*> USuqsTaskState::GetWaypoints(bool bOnlyEnabled) {
  ...
  if (IsValid(GetWorld())) {
    ...
    if (IsValid(GI)) {
      auto Suqs = GI->GetSubsystem<USuqsWaypointSubsystem>();
      Suqs->GetWaypoints(GetParentObjective()->GetParentQuest()->GetIdentifier(), GetIdentifier(),
                         bOnlyEnabled, Ret);
    }
  }
  return Ret;
...
void USuqsTaskState::FinishLoad() {
  ...
  if (IsMandatory()) {
    if (IsHiddenOnCompleteOrFail() && (IsCompleted() || IsFailed())) { bHidden = true; } else {
      ...
      if (Obj->AreTasksSequential() && Obj->GetNextMandatoryTask() != this) { bHidden = true; }
    }
  }
