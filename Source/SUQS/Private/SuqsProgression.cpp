  ...
  for (const FString& Path : Paths) {
    ...
    for (auto Asset : Assets) {
      if (auto DataTable = Cast<UDataTable>(Asset)) {
        if (DataTable->RowStruct == FSuqsQuest::StaticStruct()) { DataTables.Add(DataTable); }
      }
    }
  }
...
const TMap<FName, FSuqsQuest>& USuqsProgression::GetQuestDefinitions(bool bForceRebuild) {
  if (bForceRebuild) { RebuildAllQuestData(); }
...
bool USuqsProgression::AcceptQuest(FName QuestID, bool bResetIfFailed, bool bResetIfComplete,
                                   bool bResetIfInProgress) {
  ...
  if (QDef) {
    ...
    if (Quest) {
      ...
      if (!bReset) {
        UE_LOG(LogSUQS, Warning,
               TEXT("Ignoring request to accept quest %s because it has status %d"),
               *QuestID.ToString(), Quest->Status);
        return false;
      }
      ...
      // The problem is that initialisation could trigger detail events while it sorts itself out,
      // out of order Let's suppress that
      ...
      if (!bSuppressEvents) {
        ...
        OnProgressionEvent.Broadcast(
            FSuqsProgressionEventDetails(ESuqsProgressionEventType::QuestAccepted, Quest));
        OnActiveQuestsListChanged.Broadcast();
        OnProgressionEvent.Broadcast(
            FSuqsProgressionEventDetails(ESuqsProgressionEventType::ActiveQuestsChanged));
...
bool USuqsProgression::CompleteTask(FName QuestID, FName TaskIdentifier) {
  ...
  UE_LOG(LogSUQS, Warning,
         TEXT("Attempted to complete task %s/%s but it was not found in the active quests"),
...
USuqsObjectiveState* USuqsProgression::GetCurrentObjective(FName QuestID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->GetCurrentObjective(); }
  return nullptr;
}


bool USuqsProgression::IsQuestAccepted(FName QuestID) const {
  if (FindQuestState(QuestID)) { return true; }
  return false;
}
...
bool USuqsProgression::IsQuestIncomplete(FName QuestID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsIncomplete(); }
  return true;
}

bool USuqsProgression::IsQuestCompleted(FName QuestID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsCompleted(); }
  return false;
}

bool USuqsProgression::IsQuestFailed(FName QuestID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsFailed(); }
  return false;
}

bool USuqsProgression::IsObjectiveIncomplete(FName QuestID, FName ObjectiveID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsObjectiveIncomplete(ObjectiveID); }
  return false;
}

bool USuqsProgression::IsObjectiveCompleted(FName QuestID, FName ObjectiveID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsObjectiveCompleted(ObjectiveID); }
  return false;
}

bool USuqsProgression::IsObjectiveFailed(FName QuestID, FName ObjectiveID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsObjectiveFailed(ObjectiveID); }
  return false;
}

void USuqsProgression::ResetObjective(FName QuestID, FName ObjectiveID) {
  if (auto Q = FindQuestState(QuestID)) { Q->ResetObjective(ObjectiveID); }
}

USuqsTaskState* USuqsProgression::GetNextMandatoryTask(FName QuestID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->GetNextMandatoryTask(); }
  return nullptr;
}


bool USuqsProgression::IsTaskIncomplete(FName QuestID, FName TaskID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsTaskIncomplete(TaskID); }
...
bool USuqsProgression::IsTaskCompleted(FName QuestID, FName TaskID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsTaskCompleted(TaskID); }
  return false;
}

bool USuqsProgression::IsTaskFailed(FName QuestID, FName TaskID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsTaskFailed(TaskID); }
  return false;
}

bool USuqsProgression::IsTaskRelevant(FName QuestID, FName TaskID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsTaskRelevant(TaskID); }
  return false;
}

USuqsTaskState* USuqsProgression::GetTaskState(FName QuestID, FName TaskID) const {
  if (auto Q = FindQuestState(QuestID)) { return Q->GetTask(TaskID); }
  return nullptr;
}

void USuqsProgression::ResetTask(FName QuestID, FName TaskID) {
  if (auto Q = FindQuestState(QuestID)) { Q->ResetTask(TaskID); }
}

void USuqsProgression::SetQuestBranchActive(FName QuestID, FName Branch, bool bActive) {
  if (auto Q = FindQuestState(QuestID)) { Q->SetBranchActive(Branch, bActive); }
}


bool USuqsProgression::IsQuestBranchActive(FName QuestID, FName Branch) {
  if (auto Q = FindQuestState(QuestID)) { return Q->IsBranchActive(Branch); }
  return false;
}
...
void USuqsProgression::RemoveAllParameterProviders() { ParameterProviders.Empty(); }
...
FText USuqsProgression::FormatTaskText(const FName& QuestID, const FName& TaskID,
                                       const FText& FormatText) {
...
bool USuqsProgression::GetTextNeedsFormatting(const FText& Text) {
  ...
  // This picks up both ordered and named, but we don't support ordered for simplicity (and it's
...
void USuqsProgression::RaiseTaskUpdated(USuqsTaskState* Task) {
  ...
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::TaskUpdated, Task));
  }
}
...
void USuqsProgression::RaiseTaskCompleted(USuqsTaskState* Task) {
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::TaskCompleted, Task));
  }
...
void USuqsProgression::RaiseTaskAdded(USuqsTaskState* Task) {
  if (!bSuppressEvents) {
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::TaskAdded, Task));
  }
  // A task being added means its waypoints become relevant, so we should get events for them being
  // changed
  auto Waypoints = Task->GetWaypoints(false);
  for (auto W : Waypoints) {
    W->SetIsCurrent(true);
    ...
    if (!bSubcribedToWaypointEvents) {
      ...
      if (IsValid(GI)) {
        ...
        Suqs->OnAnyWaypointEnabledChanged.AddDynamic(this,
                                                     &USuqsProgression::OnWaypointEnabledChanged);
...
void USuqsProgression::RaiseTaskRemoved(USuqsTaskState* Task) {
  if (!bSuppressEvents) {
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::TaskRemoved, Task));
  }
...
void USuqsProgression::RaiseTaskFailed(USuqsTaskState* Task) {
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::TaskFailed, Task));
  }
...
void USuqsProgression::RaiseObjectiveCompleted(USuqsObjectiveState* Objective) {
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::ObjectiveCompleted, Objective));
  }
}
...
void USuqsProgression::RaiseObjectiveFailed(USuqsObjectiveState* Objective) {
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::ObjectiveFailed, Objective));
  }
}
...
void USuqsProgression::RaiseQuestCompleted(USuqsQuestState* Quest) {
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::QuestCompleted, Quest));
  }

  // We don't process changes caused by complete / fail until barriers resolved (see
...
void USuqsProgression::RaiseQuestFailed(USuqsQuestState* Quest) {
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::QuestFailed, Quest));
  }
...
void USuqsProgression::RaiseCurrentObjectiveChanged(USuqsQuestState* Quest) {
  if (!bSuppressEvents) {
    OnProgressionEvent.Broadcast(FSuqsProgressionEventDetails(
        ESuqsProgressionEventType::QuestCurrentObjectiveChanged, Quest));
...
void USuqsProgression::ProcessQuestStatusChange(USuqsQuestState* Quest) {
  ...
  if (Status == ESuqsQuestStatus::Completed || Status == ESuqsQuestStatus::Failed) {
    ...
    if (!bSuppressEvents) {
      OnProgressionEvent.Broadcast(
          FSuqsProgressionEventDetails(ESuqsProgressionEventType::QuestArchived, Quest));
...
FSuqsResolveBarrier USuqsProgression::GetResolveBarrierForQuest(const FSuqsQuest* Quest,
                                                                ESuqsQuestStatus Status) const {
  FSuqsResolveBarrier Barrier;
...
void USuqsProgression::LoadFromData(const FSuqsSaveData& Data) {
  // Save / load from data is deliberately self-contained here in progression and not distributed
  // around the quest / objective / task classes. Partly this is because we compress out Objectives
  // in the save data and partly it's because it's just easier to follow, considering it's only a
  // few lines of code
  ActiveQuests.Empty();
  QuestArchive.Empty();
  GlobalActiveBranches.Empty();
  ...
  for (auto& QData : Data.QuestData) {
    if (auto QDef = GetQuestDefinition(FName(QData.Identifier))) {
      ...
      // This will re-create the quest structure, including objectives and tasks, based on *current*
      // definition
      ...
      for (auto& TData : QData.TaskData) {
        ...
        if (auto T = Q->GetTask(FName(TData.Identifier))) {
          ...
          // It's important this is done LAST, because completion triggered from the above can
          // generate a new barrier
      ...
      UE_LOG(LogSUQS, Warning,
             TEXT("Ignoring saved quest data for %s because that quest no longer exists"),
             *QData.Identifier);
    }
  }
...
void USuqsProgression::OnWaypointMoved(USuqsWaypointComponent* Waypoint) {
  if (!bSuppressEvents) {
    ...
    OnProgressionEvent.Broadcast(
        FSuqsProgressionEventDetails(ESuqsProgressionEventType::WaypointMoved, Waypoint, Task));
  }
}
