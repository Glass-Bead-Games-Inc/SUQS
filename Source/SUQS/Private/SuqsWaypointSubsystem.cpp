...
void USuqsWaypointSubsystem::RegisterWaypoint(USuqsWaypointComponent* Waypoint) {
  ...
  // group by task, then if there's >1 waypoint for a single task, order by sequence index for later
  // convenience
  ...
  if (bInserted) {
    ...
    // This isn't saved in the waypoint savegame data because it's derived from quest progression
    // and may be out of date from when it was last saved (making progress while the level is
    // unloaded)
    if (Progression.IsValid()) {
      Waypoint->SetIsCurrent(
          Progression->IsTaskRelevant(Waypoint->GetQuestID(), Waypoint->GetTaskID()));
    }
    Waypoint->OnWaypointMoved.AddDynamic(this, &USuqsWaypointSubsystem::OnWaypointMoved);
    Waypoint->OnWaypointEnabledChanged.AddDynamic(
        this, &USuqsWaypointSubsystem::OnWaypointEnabledChanged);
    Waypoint->OnWaypointIsCurrentChanged.AddDynamic(
        this, &USuqsWaypointSubsystem::OnWaypointIsCurrentChanged);
  }
}
...
void USuqsWaypointSubsystem::UnregisterWaypoint(USuqsWaypointComponent* Waypoint) {
  ...
  if (pList) {
    ...
    Waypoint->OnWaypointEnabledChanged.RemoveDynamic(
        this, &USuqsWaypointSubsystem::OnWaypointEnabledChanged);
    Waypoint->OnWaypointIsCurrentChanged.RemoveDynamic(
        this, &USuqsWaypointSubsystem::OnWaypointIsCurrentChanged);
  }
}

USuqsWaypointComponent*
USuqsWaypointSubsystem::GetWaypoint(const FName& QuestID, const FName& TaskID, bool bOnlyEnabled) {
  const auto pList = WaypointsByQuest.Find(QuestID);
  if (pList) {
    for (auto W : *pList) {
      if (W->GetTaskID() == TaskID && (!bOnlyEnabled || W->IsEnabled())) {
...
bool USuqsWaypointSubsystem::GetWaypoints(const FName& QuestID, const FName& TaskID,
                                          bool bOnlyEnabled,
    ...
    for (auto W : *pList) {
      if (W->GetTaskID() == TaskID) {
        ...
        if (!bOnlyEnabled || W->IsEnabled()) {
          ...
          OutWaypoints.Add(W);
          // we won't use OutWayPoints.Num() to determine if we found anything, because we're not
          // clearing the list to give callers the flexibility of appending to an existing list if
          // they want
          bAnyFound = true;
        }
      } else if (bFoundTask) {
