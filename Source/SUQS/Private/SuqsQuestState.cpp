...
void USuqsQuestState::Initialise(const FSuqsQuest* Def, USuqsProgression* Root) {
  ...
  // That means there's never a chance that the state doesn't match the definitions (breaking
  // changes to quests will have to be handled another way)
  ...
  bActiveDescriptionNeedsFormatting =
      USuqsProgression::GetTextNeedsFormatting(QuestDefinition->DescriptionWhenActive);
  bCompletedDescriptionNeedsFormatting =
      USuqsProgression::GetTextNeedsFormatting(QuestDefinition->DescriptionWhenCompleted);
...
void USuqsQuestState::Tick(float DeltaTime) {
  // We tick our own conditions FIRST, otherwise ticking children could change our status and tick
  // us simultaneously
...
bool USuqsQuestState::CompleteTask(FName TaskID) {
  if (auto T = GetTask(TaskID)) { return T->Complete(); }
  return false;
}
...
void USuqsQuestState::Fail() {
  auto Obj = GetCurrentObjective();
  if (Obj) { Obj->FailOutstandingTasks(); }
}
...
void USuqsQuestState::NotifyObjectiveStatusChanged() {
  // Re-scan the objectives from top to bottom (this allows ANY change to have been made, including
  // backtracking) The next active objective is the next incomplete one in sequence which is on an
  // active branch If there is no next objective, then the quest is complete.
  ...
  if (FailQuest) { ChangeStatus(ESuqsQuestStatus::Failed); } else if (CurrentObjectiveIndex == -1) {
    ...
    if (PrevObjIndex != CurrentObjectiveIndex) {
      if (!bSuppressObjectiveChangeEvent) {
        // Raise task removed for any non-hidden tasks, these would not have received a notification
        // before, either because they're incomplete (optional), or non-sequential from the previous
        // objective This is for convenience
        if (Objectives.IsValidIndex(PrevObjIndex)) {
          ...
          for (const auto PrevTask : PrevObj->GetTasks()) {
            if (!PrevTask->GetHidden()) { Progression->RaiseTaskRemoved(PrevTask); }
          }
        }
        Progression->RaiseCurrentObjectiveChanged(this);
      }
...
void USuqsQuestState::MaybeNotifyStatusChange() {
  ...
  if (IsResolveBlockedOn(ESuqsResolveBarrierCondition::Time)) {
    if (ResolveBarrier.TimeRemaining > 0) { bCleared = false; }
  }
  if (IsResolveBlockedOn(ESuqsResolveBarrierCondition::Gate)) {
    if (!Progression->IsGateOpen(ResolveBarrier.Gate)) { bCleared = false; }
  }
