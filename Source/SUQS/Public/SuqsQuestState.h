...
struct FSuqsResolveBarrier {
  ...
  friend bool operator==(const FSuqsResolveBarrier& A, const FSuqsResolveBarrier& B) {
    return A.Conditions == B.Conditions && A.TimeRemaining == B.TimeRemaining && A.Gate == B.Gate &&
        A.bGrantedExplicitly == B.bGrantedExplicitly && A.bPending == B.bPending;
  }
  ...
  FSuqsResolveBarrier(const FSuqsResolveBarrierStateData& B) { *this = B; }
};
...
public:
  ...
  /// Return the list of ALL objectives. If you only want active objectives (branching), use
  /// GetActiveObjectives
  ...
  /// Set an objective branch to be active in this quest. Objectives associated with this branch
  /// will then be allowed to activate.
  ...
  /**
   * Increment task progress, if it exists. Increases the number value on a task, clamping it to the
   * min/max numbers in the quest definition. If this increment takes the task number to the target,
   * it completes the task as per CompleteTask.
   * @param TaskID The identifier of the task within the quest
   * @param Delta The change to make to the number on the task
   * @returns The number of "things" outstanding on the task after progress was applied (0 if not
   * found)
   */
  UFUNCTION(BlueprintCallable)
  ...
  /// Find a given objective by its identifier. Remember, Objective identifiers are optional so if
  /// you haven't given it an identifier, it won't show up here.
  UFUNCTION(BlueprintCallable)
  ...
  /// Return whether this quest is completed/failed but is blocked from resolving because of an
  ...
  /// will be the first one. Check the current objective for more details. May return nullptr if
  /// there is nothing to do next right now (may occur due to e.g. barriers)
  UFUNCTION(BlueprintCallable)
  ...
  /// Manually fail this quest. Normally you should fail a specific task, but if you call this then
  /// it will mark the current task(s) failed and then fail this quest. Does nothing if the quest
  /// doesn't have outstanding tasks
  UFUNCTION(BlueprintCallable)
  void Fail();

  /// Manually complete all the required mandatory tasks on this quest. You should *really* be
  /// completing tasks instead of calling this, but if you need it, this bypasses that marks all
  /// mandatory tasks as complete
  UFUNCTION(BlueprintCallable)
  void Complete();
