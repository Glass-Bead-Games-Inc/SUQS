...
/// Identifies the type of quest event that has occurred, for those who want to listen in to a
/// single event source
UENUM(BlueprintType)
enum class ESuqsProgressionEventType : uint8 {
  /// Raised when the list of active quests changes (quests may not be removed from the list
  /// immediately on completion/failure)
  ActiveQuestsChanged,
  /// Raised when a quest is archived because it was previously completed or failed, details include
  /// quest link
  QuestArchived,
  ...
  ObjectiveFailed,
  /// Raised when a new task has been added to the list of relevant ones to be displayed within the
  /// current objective Details include  task and quest
  TaskAdded,
  /// Raised when some detail on the task changes - progress made, time changed etc. NOT raised for
  /// status changes e.g. completion (but changes may cause them)
  TaskUpdated,
  ...
  TaskFailed,
  /// Raised when a task has been removed from the list of relevant ones to be displayed within the
  /// current objective This could be because the task has been completed/failed, or that it's
  /// optional and the objective has changed without completing Details include  task and quest
  TaskRemoved,
...
/// Details to go with the general progression event, not all members will be valid, see
/// ESuqsProgressionEventType
...
/**
 * Progression holds all the state relating to all quests and their objectives/tasks for a single
 * player. Add this somewhere that's useful to you, e.g. your PlayerState or GameInstance. And of
 * course, you'll want to include it in your save games. You MUST only ever have one instance of
 * this in your game.
 */
...
public:
  ...
  UFUNCTION(BlueprintCallable)
  void AddQuestDataTables(TArray<UDataTable*> Tables);
  void AddQuestDataTable(UDataTable* Tables);

  /// Initialise this progress instance with quest definitions contained in all datatables found in
  /// a given content location. Path must be of the form "/Game/Sub/Folder/Within/Content". Will
  /// recurse into subdirectories. You must also call this BEFORE calling any other function
  UFUNCTION(BlueprintCallable)
  void InitWithQuestDataTablesInPath(const FString& Path);

  /// Initialise this progress instance with quest definitions contained in all datatables found in
  /// a given content locations. Paths must be of the form "/Game/Sub/Folder/Within/Content". Will
  /// recurse into subdirectories. You must also call this BEFORE calling any other function
  UFUNCTION(BlueprintCallable)
  void InitWithQuestDataTablesInPaths(const TArray<FString>& Paths);

  /**
   * Change the default time delays between completing / failing a quest item, and the knock-on
   * effects of that (the next task/objective/quest being activated). Where one completion rolls
   * into another, the time delays are in serial. Therefore when completing the last task in a
   * quest, the task delay will tick by before the objective is completed, which will complete the
   * quest and then there can be another delay before any dependent quests are activated.
   *
   * Objectives don't have a delay of their own since they're just groupings of tasks.
   *
   * @param QuestDelay The time between a quest being completed/failed, and the effects on dependent
   * quests (default 2)
   * @param TaskDelay The time between a task being completed/failed, and dependent effects (next
   * task, objective complete etc) (default 2)
   */
  UFUNCTION(BlueprintCallable)
  void SetDefaultProgressionTimeDelays(float QuestDelay, float TaskDelay);

  /// This single event is best for quest UIs since it can give details about any relevant change in
  /// quest state
  ...
  /// Fired when something on the detail of a task has changed (progress, time etc). NOT raised for
  /// status changes e.g. completion (but changes may cause them)
  ...
  /// Note: because of timing issues around parameter providers, you probably also want to subscribe
  /// to the OnParameterProvidersChanged event as well, since that can change text depending on when
  /// you query
  UPROPERTY(BlueprintAssignable)
  ...
  /// Use this from C++ to receive access to the loaded quest data before it's applied to this
  /// progression You can therefore change the quest data if you need to adapt it due to quest
  /// changes
  FOnPreLoad OnPreLoad;


  /**
   * Return the quest definitions available. These are separate to the list of accepted quests or
   * quest archive and represents all of the quests available.
   * @param bForceRebuild If true, force the internal rebuild of quest definitions. Only needed if
   * you have changed the QuestDataTables property at runtime after using other methods on this
   * instance, which is highly discouraged.
  ...
  /// Return whether the quest is or has been accepted for the player (may also be completed /
  /// failed)
  UFUNCTION(BlueprintCallable)
  bool IsQuestAccepted(FName QuestID) const;

  /// Return whether a quest is active, i.e. accepted and still in the active list. It may be
  /// completed / failed but not resolved yet (resolve moves it to the archive potentially later)
  UFUNCTION(BlueprintCallable)
   ...
   * Note: you don't need to do this for quests which are set to auto-accept based on the completion
   * of other quests. However you will want to do it for events that you activate other ways, e.g.
   * entering areas, talking to characters
   * @param QuestID The identifier of the quest
   * @param bResetIfFailed If this quest has failed, whether to reset it (default true)
   * @param bResetIfComplete If this quest has been previously completed, whether to reset it.
   * Default false (do nothing)
   * @param bResetIfInProgress If this quest is already in progress, whether to reset it. If not, do
   * nothing
   * @returns Whether the quest was successfully accepted
   */
  UFUNCTION(BlueprintCallable)
  bool AcceptQuest(FName QuestID, bool bResetIfFailed = true, bool bResetIfComplete = false,
                   bool bResetIfInProgress = false);

  /// Reset all progress on a quest. Works whether a quest is in progress, complete or failed. Quest
  /// will remain accepted & incomplete Note: this does NOT undo prior auto-acceptance of other
  /// quests which were dependent on this quest's completion/failure If you need those other quests
  /// to reset or be removed if undoing completion/failure, you must do it manually.
  UFUNCTION(BlueprintCallable)
   ...
   * @param bRemoveArchived Whether archived (failed/completed) quests should be removed (default
   * true)
   */
  UFUNCTION(BlueprintCallable)
  ...
  /// Manually fail a quest. You should prefer using FailTask() instead if you need to explain which
  /// specific part of a quest failed. Otherwise, this will mark all current tasks /objectives as
  /// failed.
  UFUNCTION(BlueprintCallable)
  void FailQuest(FName QuestID);

  /// Manually complete a quest. You should prefer using CompleteTask() instead of using this, since
  /// that will automatically bubble up. This will mark all mandatory tasks as complete
  UFUNCTION(BlueprintCallable)
  void CompleteQuest(FName QuestID);

  /// Resolve the outcome of a completed/failed quest; move it to the quest archive and process the
  /// knock-on effects such as activating dependent quests. You do not normally need to call this,
  /// quests resolve automatically on completion/failure by default. However if the quest definition
  /// sets "ResolveAutomatically" to false then you have to call this to resolve it. Has no effect
  /// on quests which are incomplete.
  UFUNCTION(BlueprintCallable)
  void ResolveQuest(FName QuestID);

  /**
   * Mark a task as failed. If this is a mandatory task, it will fail the objective the task is
   attached to. If the objective is mandatory, it will fail the quest.
   * @param QuestID The ID of the quest. If None, will scan all active quests and fail any task with
   TaskIdentifier
  ...
  /**
   * Fully complete a task. If this is the last mandatory task in an objective, also completes the
   * objective, and cascades upwards to the quest if that's the last mandatory objective.
   * @param QuestID The ID of the quest. If None, will scan all active quests and complete any task
   * with TaskIdentifier
  ...
  /**
   * Increment task progress. Increases the number value on a task, clamping it to the min/max
   * numbers in the quest definition. If this increment takes the task number to the target, it
   * completes the task as per CompleteTask.
   * @param QuestID The ID of the quest. If None, will scan all active quests and progress any task
   * with TaskIdentifier
   * @param TaskIdentifier The identifier of the task within the quest
  ...
  /**
   * Resolve the outcome of a completed/failed task; activate the next task, or complete/fail the
   * quest if it's the last. You do not normally need to call this, tasks resolve automatically on
   * completion/failure by default. However if the task definition sets "ResolveAutomatically" to
   * false then you have to call this to resolve it. Has no effect on tasks which are incomplete.
   * @param QuestID The ID of the quest. If None, will scan all active quests and resolve any task
   * with TaskIdentifier
   * @param TaskIdentifier The identifier of the task within the quest (required)
   */
  UFUNCTION(BlueprintCallable)
  ...
  /// Return whether a given objective is incomplete ie not failed or completed. For more
  /// information, retrieve the objective itself
  UFUNCTION(BlueprintCallable)
  bool IsObjectiveIncomplete(FName QuestID, FName ObjectiveID) const;

  /// Return whether a given objective is completed. For more information, retrieve the objective
  /// itself
  UFUNCTION(BlueprintCallable)
  ...
  /// If the objective for this quest only requires ONE of a number of tasks to be completed, this
  /// will be the first one. Check the current objective for more details.
  UFUNCTION(BlueprintCallable)
  USuqsTaskState* GetNextMandatoryTask(FName QuestID) const;

  /// Return whether a given task is incomplete ie not failed or completed. For more information,
  /// retrieve the task itself
  UFUNCTION(BlueprintCallable)
  ...
  /// Set a branch to be active in a specific quest. Objectives in this quest that are associated
  /// with this branch will then be allowed to activate.
  UFUNCTION(BlueprintCallable)
  ...
  /// Set a branch to be active globally for ALL quests. For all quests, objectives associated with
  /// that branch will be active. This setting affects ALL quests, including any which are accepted
  /// after calling this
  UFUNCTION(BlueprintCallable)
  ...
  /// Set a whether a given progression gate is open. Gates are unique names which prevent automatic
  /// progression on completion / failure of an item until that gate is set open (all gates are
  /// closed by default). This can help you manually control when a task / objective / quest rolls
  /// over to the next on completion / failure, so you can tick off items but not have things move
  /// forward until you're ready.
  UFUNCTION(BlueprintCallable)
  ...
  /// You don't usually need to call this yourself, if auto-accept is enabled on your quests. But if
  /// you want to determine it manually you can.
  UFUNCTION(BlueprintCallable)
  bool QuestDependenciesMet(const FName& QuestID);

  /**
   * Add an object which can provide parameter values
   * @param Provider An object which implements ISuqsParameterProvider. Will be held by WEAK
   * pointer!
   */
  UFUNCTION(BlueprintCallable)
