...
UENUM(BlueprintType)
enum class ESuqsQuestDataStatus : uint8 { Incomplete = 0, Completed = 2, Failed = 4 };
 ...
 * We ONLY store quests and tasks, not objectives. Objective data is implied from the quest
 * definitions so we only need to store the state of tasks (and quests as a top level since tasks
 * IDs are only unique within quests)
 */
USTRUCT(BlueprintType)
