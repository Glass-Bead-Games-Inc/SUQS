 ...
 * Public exposure of these details is on USuqsProgression, which will listen to the events this
 * class raises.
 */
UCLASS()
...
public:
  /// Raised when a currently registered waypoint moves, if it has movement events enabled and is
  /// current It's generally better to attach a visual to the same actor as the waypoint component
  /// instead, but you might need this
  ...
  /// Raised when any currently registered waypoint becomes or stops being current (associated with
  /// an active task)
  UPROPERTY(BlueprintAssignable)
  ...
  /**
   * @brief Get all waypoints for a task. If there are multiple, they will be ordered by sequence
   * index
   * @param QuestID The quest identifier (required)
   * @param TaskID The task identifier (required)
   * @param bOnlyEnabled Only consider enabled waypoints
   ...
   */
  bool GetWaypoints(const FName& QuestID, const FName& TaskID, bool bOnlyEnabled,
