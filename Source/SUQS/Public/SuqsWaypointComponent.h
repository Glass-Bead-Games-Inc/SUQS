...
/**
 * Component which represents a world waypoint for a quest task. They are loosely associated with
 * quest data but at at runtime when a level is loaded, any waypoints in the level will be
 * registered and made available for querying. Add a component of this type to your actors to hint
 * to the player where quests can be progressed.
 *
 * Every waypoint must be associated with a QuestID and TaskID. You can have more than one waypoint
 * for the same task, if for example you can do any of a number of things, or there are multiple
 * places in the world to achieve a task. You can also specify a sequence if those multiple
 * waypoints should be in a sequence within a task (such as a path - consider splitting the task if
 * the waypoint sequence involves different actions).
 *
 * Waypoints can be enabled/disabled, and move at runtime.
 *
 * See the USuqsProgression class for details on how to query for waypoints and to get events about
 * waypoint changes.
 */
...
protected:
  ...
  /// Default false since you don't need this if you attach this waypoint component and visual to
  /// the same actor
...
public:
  ...
  /// Event raised when this waypoint moves, but only if bRaiseMoveEvents is true, and it's current
