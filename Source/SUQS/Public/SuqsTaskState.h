...
public:
  ...
  /**
   * Advance the number associated with progress on this quest. If it reaches the target number or
   * more, it will automatically complete
  ...
  /// Get the number of "things" still left to do, will only be > 1 if TargetNumber on the task was
  /// > 1
  UFUNCTION(BlueprintCallable)
  ...
  bool IsIncomplete() const {
    return Status != ESuqsTaskStatus::Completed && Status != ESuqsTaskStatus::Failed;
  }
  ...
  /// Return whether this task is "relevant" i.e. it's on the current objective, incomplete and next
  /// in line
  UFUNCTION(BlueprintCallable, BlueprintPure)
  ...
  /// Return whether this task is completed/failed but is blocked from resolving because of an
  ...
  /**
   * Return whether this task will become hidden on completion/failure or not. Reasons not to are
   * that it's optional or the tasks are non-sequential, so stick around until the objective is
   * completed/failed
   */
