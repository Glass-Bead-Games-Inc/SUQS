...
public:
  template <typename T>
  void SetParameter(const FString& Name, const T& Value) {
  ...
  UFUNCTION(BlueprintCallable)
  void SetAllParameters(const USuqsNamedFormatParams* SourceArgs) {
    NamedArgs.Empty();
    NamedArgs.Append(SourceArgs->NamedArgs);
  }

  void Empty() { NamedArgs.Empty(); }
  FText Format(const FText& FormatText) const { return FText::Format(FormatText, NamedArgs); }
};
 ...
 * You can include ordered or named parameters in quest text, and when they're detected, any
 * requests for that text will cause a request to be issued to all of the implementors of this
 * interface which are registered with the USuqsProgression instance.
 */
...
public:
  /**
   * Callback to provide named parameters for a given top-level Quest title or description. This
   * callback will only be called if named parameters are needed.
   * @param QuestID The quest ID; will always be provided
   * @param TaskID The task ID: may be None if this is for the root quest, or a task ID if it's for
   * a specific task
   * @param Params Use this object to set the named parameters you need
   */
  UFUNCTION(BlueprintNativeEvent)
  void GetQuestParameters(const FName& QuestID, const FName& TaskID,
